  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <span
          className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-semibold ${dispute.slaTone}`}
        >
          <Clock size={12} />
          {dispute.sla}
        </span>
...
function ActionButton({ children, icon: Icon, tone = "primary", onClick }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold transition-all ${tones[tone]}`}
    >
      {Icon && <Icon size={16} />}
      {children}
    </button>
...
function Returns_B() {
  ...
  return (
    <div className="min-h-full bg-gray-50 p-6">
      {/* Top bar */}
      <div className="mb-5 flex items-center gap-3 rounded-2xl border border-gray-200 bg-white p-3 shadow-sm">
        <SEARCH
          value={query}
          onChange={setQuery}
          ...
          className="flex-1"
        />
        <span className="hidden shrink-0 items-center gap-1.5 rounded-xl border border-gray-200 px-3 py-2 text-sm font-medium text-slate-500 sm:inline-flex">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div>
              ...
              <div className="mt-3 grid grid-cols-3 gap-2">
                {active.evidence.map((photo) => (
                  <div
                    key={photo}
                    className="flex aspect-square items-center justify-center rounded-xl bg-slate-100 text-xs font-medium text-slate-400"
                  >
                    {photo}
                  </div>
                ))}
          ...
          <div className="mt-6">
            ...
            <textarea
              ...
              rows={3}
              className="mt-3 w-full resize-none rounded-xl border border-gray-200 bg-gray-50 p-3 text-sm text-slate-700 outline-none transition focus:border-sky-300 focus:bg-white focus:ring-2 focus:ring-sky-100"
            />
            <div className="mt-3 flex flex-wrap items-center gap-5 text-xs font-medium text-slate-500">
              <button
                type="button"
                className="inline-flex items-center gap-1.5 hover:text-slate-700"
              >
                <Paperclip size={14} /> Attach POD
              </button>
              <button
                type="button"
                className="inline-flex items-center gap-1.5 hover:text-slate-700"
          ...
          <div className="mt-6 flex flex-wrap items-center justify-between gap-3 border-t border-gray-100 pt-5">
            ...
            <button
              type="button"
              className="inline-flex items-center gap-1.5 text-sm font-semibold text-slate-500 hover:text-red-500"
            >
              <ShieldOff size={16} /> Reject dispute
            </button>
          </div>
        </div>
      </div>
